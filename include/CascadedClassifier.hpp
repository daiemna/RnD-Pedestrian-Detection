...
public:
	CascadedClassifier(unsigned int cascadeCount, CvStatModel classifier);
	~CascadedClassifier();
	void clear();
	...
	bool train(const Mat& train_data, const Mat& responses, int tflag =
			CV_COL_SAMPLE, const Mat& var_idx = 0, const Mat& sample_idx = 0);
	float predict(const Mat& sample) const;
private:
//		unsigned int cascadeCount;
	vector<CvStatModel> cascade_list;
	Mat featureIndices;
	Mat sampleIndices;
